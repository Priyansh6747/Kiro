...
function ScheduledTask({ task, onClick, isSelected }: { task: Task; onClick: () => void; isSelected: boolean }) {
  return (
    <div 
      onClick={onClick}
    ...
    >
      <div className="flex justify-between items-center mb-1">
        <span className="text-sm font-medium text-primary line-clamp-1 mr-2">{task.title}</span>
      </div>
...
export function ScheduledTimelineColumn({
  ...
  const scheduledTasks = tasks.filter(t => t.scheduledDate && t.status !== 'deleted');
  
  const groupedScheduled = scheduledTasks.reduce((acc, t) => {
  ...
  const sortedDates = Object.keys(groupedScheduled).map(Number).sort((a, b) => a - b);
  ...
  const renderTimeline = () => {
    ...
    if (timelineMode === 'continuous') {
      ...
      let minDateNum = todayNum;
      let maxDateNum = todayNum;

      if (sortedDates.length > 0) {
        minDateNum = Math.min(todayNum, sortedDates[0]);
        maxDateNum = Math.max(todayNum, sortedDates[sortedDates.length - 1]);
      }

      const minDate = parseDateStr(minDateNum);
      minDate.setDate(minDate.getDate() - 7);
      const maxDate = parseDateStr(maxDateNum);
          ...
          >
            <div className="w-20 flex-shrink-0 flex flex-col items-center justify-center relative">
              ...
              <div className="flex flex-col items-center justify-center bg-surface px-1 py-1 relative z-10 mt-[-10px]">
                <span className={`text-sm tracking-wide ${isToday ? 'text-accent font-bold' : 'text-primary font-medium'}`}>{formatDateShort(curr)}</span>
                {isToday && <span className="text-[10px] text-accent uppercase font-bold mt-0.5">Today</span>}
              </div>
            </div>
            <div className="flex-1 p-2 space-y-2 border-l border-border-default ml-[-1px]">
              {dayTasks.map(t => (
                <ScheduledTask key={t.id} task={t} onClick={() => onSelectTask(t)} isSelected={selectedTask?.id === t.id} />
              ))}
            </div>
          </div>
        );
      ...
      if (sortedDates.length === 0) return <div className="p-4 text-secondary text-sm">No scheduled tasks.</div>;
      for (let i = 0; i < sortedDates.length; i++) {
        ...
        const renderDay = (n: number, isMain: boolean) => (
          <div key={`${dNum}-${n}`} className="flex min-h-[80px] relative">
            <div className="w-20 flex-shrink-0 flex flex-col items-center justify-center relative">
              <TimelineTicks />
              <div className="flex flex-col items-center justify-center bg-surface px-1 py-1 relative z-10 mt-[-10px]">
                <span className={`text-sm tracking-wide ${isMain ? 'text-primary font-medium' : 'text-secondary opacity-50'}`}>
        ...
        if (!groupedScheduled[prevNum] || groupedScheduled[prevNum].length === 0) timelineItems.push(renderDay(prevNum, false));
        timelineItems.push(renderDay(dNum, true));
        if (!groupedScheduled[nextNum] || groupedScheduled[nextNum].length === 0) timelineItems.push(renderDay(nextNum, false));
        
        if (i < sortedDates.length - 1 && sortedDates[i+1] > nextNum + 1) {
          timelineItems.push(
            <div key={`break-${dNum}`} className="flex h-12 items-center bg-surface-raised relative">
              <div className="w-20 flex-shrink-0 h-full relative border-r border-border-default">
                <svg className="absolute inset-x-0 top-1/2 -mt-2 w-full h-4 text-border-strong opacity-50" preserveAspectRatio="none" viewBox="0 0 100 100">
