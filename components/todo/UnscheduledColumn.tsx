...
function SwipeableUnscheduledTask({ task, onDone, onDelete, onClick, isSelected }: { task: Task; onDone: (t: Task) => void; onDelete: (t: Task) => void; onClick: () => void; isSelected: boolean }) {
  return (
    <div className="relative overflow-hidden border-b border-border-default last:border-0 bg-surface">
      ...
      >
        ...
        <span className="text-xs text-secondary mt-1">{task.estimateMin}m • {task.status}</span>
      </motion.div>
    </div>
  );
}
...
export function UnscheduledColumn({
  ...
  const unscheduledTasks = tasks.filter(t => !t.scheduledDate && t.status !== 'deleted');
  ...
  return (
    <div className="w-1/3 border-r border-border-default flex flex-col bg-surface">
      <div className="p-4 border-b border-border-default bg-surface flex items-center justify-between shrink-0">
        ...
        <span className="bg-accent-subtle text-accent text-xs font-bold px-2 py-1 rounded-full">{unscheduledTasks.length}</span>
      </div>
      <div className="flex-1 overflow-y-auto">
        {unscheduledTasks.length === 0 ? (
          <div className="p-4 text-secondary text-sm text-center mt-10">No unscheduled tasks.</div>
        ) : (
