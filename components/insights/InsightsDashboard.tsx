...
interface InsightsDashboardProps {
  ...
  projects: Project[];
  allTasks: Task[];
  projectTaskCounts: Record<string, { total: number; done: number; pending: number }>;
  days: number;
...
export function InsightsDashboard({
  ...
  const dashboardData = useMemo(() => {
    ...
    const totalCompleted = logs.reduce((sum, log) => sum + log.tasksCompleted, 0);
    const totalCarried = logs.reduce((sum, log) => sum + log.tasksCarried, 0);
    const totalAvailableMin = logs.reduce((sum, log) => sum + log.availableMin, 0);
    
    // Approximate total assigned minutes
    const tasksInPeriod = allTasks.filter(t => t.scheduledDate && logs.some(l => l.date === t.scheduledDate));
    const assignedMin = tasksInPeriod.reduce((sum, t) => sum + (t.estimateMin || 30), 0);
    ...
    const timeOverCommit = totalAvailableMin > 0 ? assignedMin / totalAvailableMin : 0;
    ...
    const projTypeMap: Record<string, number> = { critical: 0, recurring: 0, habit: 0, nicetohave: 0 };
    ...
    const timeAllocation = Object.entries(projTypeMap)
      ...
      .map(([type, val]) => ({
        ...
        value: val,
        percentage: totalCompletedMin > 0 ? Math.round((val / totalCompletedMin) * 100) : 0
      }));
