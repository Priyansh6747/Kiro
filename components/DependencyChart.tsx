...
import { ReactFlow, Controls, Background, Edge, Node, MarkerType, Position, Handle, Connection, useNodesState, useEdgesState } from "@xyflow/react";
...
export function DependencyChart({
  ...
  const getAutoLayout = useCallback((tasksList: Task[], depsList: { taskId: string; predecessorId: string }[]) => {
    ...
    const remaining = tasksList.filter(t => !placed.has(t.id)).map(t => t.id);
    ...
    depthLevels.forEach((level, x) => {
      ...
      level.forEach((id, index) => {
        ...
        const allDepsCleared = taskDeps.length > 0 && taskDeps.every(d => taskMap.get(d.predecessorId)?.status === 'done');
        ...
        if (task.status === 'done') {
          ...
          borderTopColor = task.status === 'pending' ? 'var(--accent)' : 'var(--status-warning)';
        }
  ...
  const onConnect = useCallback((connection: Connection) => {
    if (connection.source && connection.target && onAddDependency) {
      ...
      const exists = dependencies.some(d => d.taskId === connection.target && d.predecessorId === connection.source);
      if (exists) return;
  ...
  return (
    <div className="flex flex-col gap-3">
      ...
      <div className="flex flex-wrap gap-4 items-center justify-center text-xs text-secondary py-1 bg-surface rounded-lg border border-border-default px-4 py-2 self-center">
        <div className="flex items-center gap-1.5"><div className="w-3 h-3 rounded" style={{ backgroundColor: 'var(--node-start)' }}></div> Start</div>
        <div className="flex items-center gap-1.5"><div className="w-3 h-3 rounded" style={{ backgroundColor: 'var(--node-ready)' }}></div> Ready</div>
        <div className="flex items-center gap-1.5"><div className="w-3 h-3 rounded" style={{ backgroundColor: 'var(--node-final)' }}></div> Final</div>
        <div className="flex items-center gap-1.5"><div className="w-3 h-3 rounded" style={{ backgroundColor: 'var(--status-done)' }}></div> Done</div>
        <div className="flex items-center gap-1.5"><div className="w-3 h-3 rounded" style={{ backgroundColor: 'var(--accent)' }}></div> Pending/Locked</div>
      </div>
    </div>
  );
}
