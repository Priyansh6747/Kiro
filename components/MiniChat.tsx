...
import { Bot, Maximize2, X, Send } from "lucide-react";
        ...
        <div className="fixed bottom-24 right-6 md:bottom-28 md:right-10 w-80 md:w-96 bg-surface border border-border-default rounded-2xl shadow-2xl z-50 flex flex-col overflow-hidden animate-in slide-in-from-bottom-5 fade-in duration-200">
          <div className="flex items-center justify-between p-4 border-b border-border-default bg-surface-raised">
            <div className="flex items-center gap-2">
              ...
              <span className="font-semibold text-primary">Yuki AI</span>
            </div>
                    ...
                    <div key={idx} className={`flex flex-col ${!m || m.role !== "user" ? "items-start" : "items-end"}`}>
                      {tools.length > 0 && (
                        <div className="flex flex-row flex-wrap items-center gap-1.5 mb-1 ml-1">
                          {tools.map((tc: any) => (
                            <div key={tc.id} className="flex items-center gap-1 px-2 py-1 rounded bg-accent/10 border border-accent/20 text-[10px] font-semibold text-accent shadow-sm">
                      ...
                      {m && (
                        <div className={`max-w-[90%] p-3 rounded-xl shadow-sm text-sm ${m.role === "user" ? "bg-accent text-white" : "bg-surface-raised border border-border-default text-primary"}`}>
                          {m.content ? (
                            ...
                            <div className="flex flex-col gap-2">
                              <span className="font-semibold text-xs">Action Required:</span>
                              ...
                              <div className="flex gap-1.5 mt-1">
                                <button onClick={() => executeTools(true)} className="flex-1 py-1 bg-accent text-white rounded text-xs hover:bg-accent-hover transition-colors">Allow</button>
                                <button onClick={() => executeTools(false)} className="flex-1 py-1 bg-surface border border-border-default text-secondary rounded text-xs transition-colors hover:text-primary">Deny</button>
                              </div>
                            </div>
                          ) : null}
                        </div>
