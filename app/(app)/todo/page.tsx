...
import { listProjects, listTasks, updateTask, deleteTask, getProjectDependencies, addDependency, deleteDependency } from "@/lib/api-client";
...
export default function TodoPage() {
  ...
  const [timelineMode, setTimelineMode] = useState<'compact' | 'continuous'>('continuous');
  ...
  const handleAddTask = async () => {
    ...
    try {
      ...
      const t = await createTask({ project_id: todoProject.id, title: "New Task", estimate_min: 30 });
  ...
  const removeDep = async (taskId: string, predecessorId: string) => {
    try {
      ...
      setDependencies(prev => prev.filter(d => !(d.taskId === taskId && d.predecessorId === predecessorId)));
    } catch (e) {
      showToast((e as Error).message, 'error');
    }
  };
